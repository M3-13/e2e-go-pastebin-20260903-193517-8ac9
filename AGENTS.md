...
4. **Tests are part of done.** Every ticket includes writing tests AND making
   them pass. `go test ./...` must exit 0 before you stop.
...
- **test** (`go build ./...` + `go test ./...`) — BLOCKING
- **lint** (`gofmt` + `go vet`) — warning, not blocking

Important consequences for you:
1. **`go build ./...` and `go test ./...` must be green locally.** New functions
   need tests (`*_test.go`, stdlib `testing`).
2. **Run `gofmt -w .`** before you finish.
3. **No hardcoded secrets/keys/URLs** — use env vars.
4. **Keep `go.mod` tidy** (`go mod tidy` after adding deps).
